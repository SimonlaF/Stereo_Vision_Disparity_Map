...
cv::Size pattern(4, 6);   // Nombre de coins internes du damier
...
if (runMonoCalib) {
    ...
    mono.runCalibrationFromFiles({"../left.jpeg"},"intrinsics_L.yml");
    mono.runCalibrationFromFiles({"../right.jpeg"}, "intrinsics_R.yml");
}
// =========================================================
...
if (runStereoCalib) {
    ...
    std::string fichierSortie = "stereo_params.yml";
    if (stereo.loadIntrinsics("intrinsics_L.yml", "intrinsics_R.yml")) {
        stereo.runStereoCalibrationFromTwoFiles("../left.jpeg","../right.jpeg",fichierSortie);
    } else {
