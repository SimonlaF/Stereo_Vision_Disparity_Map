...
// Function to load intrinsic parameters from files
...
// Function to generate groundtruth 3D points of the checkerboard corners
...
// Function to calibrate from live cameras
bool StereoCalibrator::runStereoCalibration(int camIdx1, int camIdx2, const std::string& saveFile) {    
    ...
    generateObjectPoints(objp);         // Groundtruth 3D points of the checkerboard corners
    ...
    std::cout << "Show the checkerboard to both cameras." << std::endl;
    ...
    while (true) {
        ...
        cv::imshow("Left Cam", vis1); cv::imshow("Right Cam", vis2);
        ...
        if (key == ' ' && found1 && found2) {
            ...
            std::cout << "Pair " << imgPts1.size() << " captured." << std::endl;
    ...
    std::cout << "Stereo Calibration done! RMS: " << rms << std::endl;
    return true;
}

// Function to calibrate from sets of images
bool StereoCalibrator::runStereoCalibrationFromFileSets(const std::vector<std::string>& leftImages,
    ...
    if (leftImages.size() != rightImages.size() || leftImages.empty()) {
        std::cerr << "Error: the number of images in both folders must match and be greater than 0" << std::endl;
        return false;
    }
    ...
    std::vector<std::vector<cv::Point2f>> imgPts1, imgPts2;
    std::vector<cv::Point3f> objp;
    generateObjectPoints(objp);     // (Groundtruth) 
    ...
    if (objectPoints.empty()) {
        std::cerr << "Error: no valid image pairs for stereo calibration" << std::endl;
        return false;
    }
...
// Function to display rectified images for visual verification of the epipolar alignment
bool StereoCalibrator::displayRectifiedView(const std::string& leftImgPath, 
                                            const std::string& rightImgPath,
                                            const std::string& stereoParamsPath) {
    // Loading parameters
    cv::FileStorage fs(stereoParamsPath, cv::FileStorage::READ);
    if (!fs.isOpened()) {
        std::cerr << "Unable to open :  " << stereoParamsPath << std::endl;
        return false;
    }
    ...
    if (frame1.empty() || frame2.empty()) {
        std::cerr << "Unable to load images." << std::endl;
        return false;
    }
    ...
    // Pre computing maps for remap
    cv::Mat map11, map12, map21, map22;
    cv::initUndistortRectifyMap(K1, D1, R1, P1, frame1.size(), CV_16SC2, map11, map12);
    cv::initUndistortRectifyMap(K2, D2, R2, P2, frame2.size(), CV_16SC2, map21, map22);

    // Applying remap to rectify the images
    cv::Mat rect1, rect2;
    cv::remap(frame1, rect1, map11, map12, cv::INTER_LINEAR);
    cv::remap(frame2, rect2, map21, map22, cv::INTER_LINEAR);

    // Concatenate the rectified images side by side for visualization
    cv::Mat canvas;
    cv::hconcat(rect1, rect2, canvas);

    // Draw horizontal lines for visual verification
    for (int i = 0; i < canvas.rows; i += 32) {
        cv::line(canvas, cv::Point(0, i), cv::Point(canvas.cols, i), cv::Scalar(0, 0, 255), 1);
    }

    cv::imshow("Rectification check", canvas);
    cv::waitKey(0); 
    return true;
}

// Function to rectify a pair of images using the computed stereo parameters to use it later for disparity computation and feature matching
void StereoCalibrator::rectifyImages(const cv::Mat& left, const cv::Mat& right,
                                     cv::Mat& rectLeft, cv::Mat& rectRight,
                                     const std::string& stereoParamsPath) {
    // Loading parameters
    cv::FileStorage fs(stereoParamsPath, cv::FileStorage::READ);
    cv::Mat K1, D1, K2, D2, R, T;
    ...
    cv::Mat R1, R2, P1, P2, Q;
    cv::stereoRectify(K1, D1, K2, D2, left.size(), R, T, R1, R2, P1, P2, Q, cv::CALIB_ZERO_DISPARITY, 0);

    // Pre computing maps for remap
    cv::Mat map11, map12, map21, map22;
    cv::initUndistortRectifyMap(K1, D1, R1, P1, left.size(), CV_16SC2, map11, map12);
    cv::initUndistortRectifyMap(K2, D2, R2, P2, right.size(), CV_16SC2, map21, map22);

    // Rectifying the images
    cv::remap(left, rectLeft, map11, map12, cv::INTER_LINEAR);
    cv::remap(right, rectRight, map21, map22, cv::INTER_LINEAR);
}